        ...
        <div className={`h-3 rounded-full overflow-hidden ${isOver || (!isComplete && sessionResults.length > 0) ? 'bg-red-100' : 'bg-gray-100'}`}>
          <div
            className={`h-full rounded-full transition-all duration-500 ${
              isOver ? 'bg-red-500' : isComplete ? 'bg-bb-green' : 'bg-amber-400'
            }`}
          ...
          <table className="w-full text-sm border-collapse">
            ...
            <tbody>
              {sessionResults.map((r, i) => (
                <tr key={r.group.id} className={`border-b border-gray-100 ${!r.isValid ? 'opacity-40' : ''}`}>
        ...
        <div className="rounded-xl bg-red-50 border border-red-200 p-4 flex items-start gap-2.5">
          ...
          <div>
            ...
            <p className="text-sm text-red-600 mt-0.5">
              ...
              {formatHours(Math.abs(remainingHours))} שעות מעל היעד ({formatHours(settings.totalHours)}).
            </p>
          </div>
        </div>
      )}
